...
nhoods <- lst(bridgeport_tracts, new_haven_tracts, hartford_tracts, stamford_tracts) %>%
  bind_rows(.id = "city") %>%
  distinct(city, town, name) %>%
  mutate(city = clean_titles(str_remove(city, "_tracts$"), cap_all = TRUE))


##################### READ STUFF ###############################################
# nhood names are in form Bridgeport_Black Rock
fetch <- readRDS(str_glue("fetch_data/acs_basic_{yr}_fetch_all.rds")) %>%
  map(label_acs, year = yr) %>%
